...
class LqtView {
	...
	*************************/
	...
	function showThreadToolbar( $thread ) {
		...
		$menuHTML = Xml::tags(
			'ul',
			array( 'class' => 'lqt-thread-toolbar-command-list' ),
			$this->listItemsForCommands( $commands )
		);

		$triggerText =	Xml::tags(
			'span',
			array( 'class' => 'lqt-thread-actions-icon' ),
			'&nbsp;'
		);
		$dropDownTrigger = Xml::tags(
			'div',
			array( 
		...
		// Box stuff
		$boxElements = array(
			'lqt-thread-toolbar-box-tl',
			'lqt-thread-toolbar-box-tr',
			'lqt-thread-toolbar-box-br',
			'lqt-thread-toolbar-box-bl'
		);
	...
	function threadInfoPanel( $thread ) {
		...
		if ( isset( $ebLookup[$editedFlag] ) ) {
			...
			$infoElements[] = Xml::element(
				'div',
				array( 'class' => 'lqt-thread-toolbar-edited-' . $editedBy ),
				$editedNotice
			);
		}
		...
		return Xml::tags(
			'div',
			array( 'class' => 'lqt-thread-info-panel' ),
			implode( "\n", $infoElements )
		);
	}
	...
	function showThread( $thread, $levelNum = 1, $totalInLevel = 1, $options = array() ) {
		...
		if ( $thread->isTopmostThread() ) {
			$html .= Xml::hidden('lqt-thre
				ad-modified-' . $thread->id(),
		...
		$this->output->addHTML( $html );
		$this->output->addHTML( Xml::openElement(
			'div',
